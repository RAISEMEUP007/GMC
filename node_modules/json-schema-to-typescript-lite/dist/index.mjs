...
function hasComment(ast) {
  return "comment" in ast && ast.comment != null && ast.comment !== "" || "deprecated" in ast && ast.deprecated === true;
}
...
const T_ANY_ADDITIONAL_PROPERTIES = {
  keyName: "[k: string]",
  type: "ANY"
};
const T_UNKNOWN = {
...
const T_UNKNOWN_ADDITIONAL_PROPERTIES = {
  keyName: "[k: string]",
  type: "UNKNOWN"
};
...
const getRootSchema = memoize((schema) => {
...
const BLACKLISTED_KEYS = /* @__PURE__ */ new Set([
  "id",
  "$defs",
  "$id",
  "$schema",
...
const generateType = memoize(generateTypeUnmemoized);
...
const rules$1 = /* @__PURE__ */ new Map();
...
rules$1.set('Remove `type=["null"]` if `enum=[null]`', (schema) => {
  if (Array.isArray(schema.enum) && schema.enum.includes(null) && Array.isArray(schema.type) && schema.type.includes("null"))
    schema.type = schema.type.filter((type) => type !== "null");
});
rules$1.set("Destructure unary types", (schema) => {
  if (schema.type && Array.isArray(schema.type) && schema.type.length === 1)
    schema.type = schema.type[0];
});
rules$1.set("Add empty `required` property if none is defined", (schema) => {
  if (isObjectType(schema) && !("required" in schema))
    schema.required = [];
});
rules$1.set("Transform `required`=false to `required`=[]", (schema) => {
  if (schema.required === false)
    schema.required = [];
});
rules$1.set("Default additionalProperties", (schema, _, options) => {
  if (isObjectType(schema) && !("additionalProperties" in schema) && schema.patternProperties === void 0)
    schema.additionalProperties = options.additionalProperties;
});
rules$1.set("Transform id to $id", (schema, fileName) => {
...
rules$1.set("Add an $id to anything that needs it", (schema, fileName, _options, _key, dereferencedPaths) => {
  if (!isSchemaLike(schema))
    return;
...
rules$1.set("Escape closing JSDoc comment", (schema) => {
  escapeBlockComment(schema);
});
rules$1.set("Add JSDoc comments for minItems and maxItems", (schema) => {
...
rules$1.set("Optionally remove maxItems and minItems", (schema, _fileName, options) => {
  if (!isArrayType(schema))
    return;
...
rules$1.set("Normalize schema.minItems", (schema, _fileName, options) => {
...
rules$1.set("Remove maxItems if it is big enough to likely cause OOMs", (schema, _fileName, options) => {
...
rules$1.set("Normalize schema.items", (schema, _fileName, options) => {
  if (options.ignoreMinAndMaxItems)
    return;
  const { maxItems, minItems } = schema;
...
rules$1.set("Remove extends, if it is empty", (schema) => {
  if (!schema.hasOwnProperty("extends"))
    return;
  if (schema.extends == null || Array.isArray(schema.extends) && schema.extends.length === 0)
    delete schema.extends;
});
rules$1.set("Make extends always an array, if it is defined", (schema) => {
  if (schema.extends == null)
    return;
  if (!Array.isArray(schema.extends))
    schema.extends = [schema.extends];
});
rules$1.set("Transform definitions to $defs", (schema, fileName) => {
...
rules$1.set("Transform const to singleton enum", (schema) => {
...
function normalize(rootSchema, dereferencedPaths, filename, options) {
  rules$1.forEach((rule) => traverse(rootSchema, (schema, key) => rule(schema, filename, options, key, dereferencedPaths)));
  return rootSchema;
}
    ...
    case "UNION":
      return {
        ...
        params: schema.type.map((type2) => {
          const member = { ...omit$1(schema, "$id", "description", "title"), type: type2 };
...
function newInterface(schema, options, processed, usedNames, keyName, keyNameFromDefinition) {
  ...
  return {
    ...
    deprecated: schema.deprecated,
    keyName,
    params: parseSchema(schema, options, processed, usedNames),
    standaloneName: name,
...
function parseSchema(schema, options, processed, usedNames, _parentSchemaName) {
...
async function compile(schema, name, options = {}) {
  ...
  const _options = merge$1({}, DEFAULT_OPTIONS, options);
...
export { DEFAULT_OPTIONS, ValidationError, compile };
